...
def find_best_existing_key(name_lower, master_stats, threshold=3):
    ...
    for existing_key in list(master_stats.keys()):
...
async def on_message(message):
    ...
    if RESULTS_CHANNEL_ID and message.channel.id == RESULTS_CHANNEL_ID:
        if message.attachments:
            valid_attachments = [a for a in message.attachments if a.filename.lower().endswith(('.png', '.jpg', '.jpeg', '.webp'))]
            ...
            try:
                for attachment in valid_attachments:
                    ...
                    hocr_data = pytesseract.image_to_data(enhanced_board, output_type=pytesseract.Output.DICT, lang='eng')
                ...
                if master_stats:
                    ...
                    for data in master_stats.values():
                        ...
                        d = max(1, data["deaths"])
                        kdr = round(k / d, 2)
                        payload = {"name": data["display_name"], "kills": k, "deaths": data["deaths"], "kdr": kdr}
                    ...
                    embed = discord.Embed(description=embed_desc, color=discord.Color.blue())
                    await processing_msg.edit(content=None, embed=embed)
                else:
                    await processing_msg.edit(content="❌ Unable to extract stats. Try clearer screenshots.")
...
async def setchannel(interaction: Interaction, type: app_commands.Choice[str]):
    ...
    try:
        if type.value == "league":
            ...
            await interaction.response.send_message(f"✅ {interaction.channel.mention} is now set as the **League Hosting** channel.", ephemeral=True)
        elif type.value == "results":
            RESULTS_CHANNEL_ID = interaction.channel.id
            await interaction.response.send_message(f"✅ {interaction.channel.mention} is now set as the **Match Results** channel.", ephemeral=True)
    except Exception as e:
...
@app_commands.describe(mode="2s, 3s, or 4s", gametype="War or Swift", perks="Perks on or off", rank="Required Rank", link="Game link")
@app_commands.choices(
    mode=[app_commands.Choice(name="2s", value="2s"), app_commands.Choice(name="3s", value="3s"), app_commands.Choice(name="4s", value="4s")],
    gametype=[app_commands.Choice(name="War", value="War"), app_commands.Choice(name="Swift", value="Swift")],
    perks=[app_commands.Choice(name="On", value="on"), app_commands.Choice(name="Off", value="off")],
    rank=[app_commands.Choice(name="Any", value="Any")] + [app_commands.Choice(name=label, value=code) for code, label in rank_labels.items()]
)
async def league(interaction: Interaction, mode: app_commands.Choice[str], gametype: app_commands.Choice[str],
                 perks: app_commands.Choice[str], rank: app_commands.Choice[str], link: str):

    if LEAGUE_CHANNEL_ID and interaction.channel.id != LEAGUE_CHANNEL_ID:
        return await interaction.response.send_message("❌ Use this in the designated League Hosting channel.", ephemeral=True)

    creator = interaction.user
    if creator.id in league_lobbies:
        return await interaction.response.send_message("❌ You already have an active league lobby.", ephemeral=True)
    ...
    channel_embed = discord.Embed(title="🏆 ACL LEAGUE", description=f"**League created by:** {creator.mention} (Rank: {user_rank_label})", color=discord.Color.blue())
    ...
    class LeagueJoinView(View):
        ...
        async def join_button(self, join_interaction: Interaction, button: Button):
            ...
            lobby = self.lobby
            if lobby.locked:
                return await join_interaction.response.send_message("❌ League is locked.", ephemeral=True)
            if user.id == lobby.owner_id:
                return await join_interaction.response.send_message("❌ You are the host.", ephemeral=True)
            if user.id in lobby.joined_users:
                return await join_interaction.response.send_message("You're already in.", ephemeral=True)
            if len(lobby.joined_users) >= lobby.max_players:
                return await join_interaction.response.send_message("❌ Lobby is full.", ephemeral=True)
            ...
            if lobby.required_rank != "Any":
                ...
                if u_rank is None or u_rank < required:
                    return await join_interaction.response.send_message(f"❌ You need rank {lobby.required_rank} or higher.", ephemeral=True)

            await lobby.thread.add_user(user)
            lobby.joined_users.append(user.id)
            await lobby.thread.send(f"{user.mention} joined ✅ (Rank: {u_label})")

            await join_interaction.response.send_message(f"✅ You're in! {lobby.thread.mention}", ephemeral=True)
    ...
    view = LeagueJoinView(None)
    leagues_role = discord.utils.get(guild.roles, name="Leagues")
    mention_text = leagues_role.mention if leagues_role else "@Leagues"

    await interaction.response.send_message(content=mention_text, embed=channel_embed, view=view, allowed_mentions=discord.AllowedMentions(roles=True))
    ...
    lobby = LeagueLobby(creator.id, creator, thread, max_players, required_rank, mode.value, gametype.value, perks.value, link)
    lobby.view_message = lobby_message
    view.lobby = lobby
    league_lobbies[creator.id] = lobby

    thread_embed = discord.Embed(title="🏆 ACL LEAGUE", description=f"**League created by:** {creator.mention} (Rank: {user_rank_label})", color=discord.Color.blue())
    ...
    await thread.send(content=f"🔗 **Game Link:** [Click here]({link})", embed=thread_embed)
    await interaction.followup.send(f"✅ Your league thread: {thread.mention}", ephemeral=True)
...
async def closelobby(interaction: Interaction):
    if interaction.user.id not in league_lobbies:
        return await interaction.response.send_message("❌ You don't have an active league lobby.", ephemeral=True)
...
async def cancelled(interaction: Interaction):
    if interaction.user.id not in league_lobbies:
        return await interaction.response.send_message("❌ You have no league to cancel.", ephemeral=True)
    lobby = league_lobbies[interaction.user.id]
    lobby.locked = True
    await send_league_log(interaction.guild, lobby, lobby.host_member)
    try:
...
async def status(interaction: Interaction):
    if interaction.user.id not in league_lobbies:
        return await interaction.response.send_message("❌ You're not hosting a league.", ephemeral=True)
    lobby = league_lobbies[interaction.user.id]
    guild = interaction.guild
    members = []
    for uid in lobby.joined_users:
    ...
    for owner_id, lobby in league_lobbies.items():
        if lobby.thread.id == interaction.channel.id:
            ...
            if len(all_players) < total_needed:
                return await interaction.response.send_message(f"❌ Need {total_needed} players for {team_size.name}, have {len(all_players)}.", ephemeral=True)
            ...
            def mentions(ids):
                return "\n".join(interaction.guild.get_member(uid).mention for uid in ids if interaction.guild.get_member(uid))
...
async def help_command(interaction: Interaction):
    ...
    embed.add_field(name="📌 Setup", value="`/setchannel [type: league/results]`", inline=False)
    embed.add_field(name="🎮 League Commands", value="`/league` `/closelobby` `/cancelled` `/leave` `/status` `/team` `/addleagueplayer` `/kickleagueplayer`", inline=False)
